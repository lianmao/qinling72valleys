...
`family_spots`（亲子，必须写强度与看护要求）、`photo_spots`（网红打卡点，只写机位与看点）。
三者同样源自 PRD §4 初稿、同样未核实。两条硬约束：

- 打卡点**主观性强、不承诺效果**，带 `warning` 的危险机位（临崖、险道、冰瀑、国道弯道、涉水站位）前端红色劝退；
- 亲子条目必须写明看护要求，低龄儿童不适合全程徒步的（如华山峪）明确写出。

## 已知待办

- [ ] 数据核实：名录权威来源、坐标逐条校对（高德坐标拾取器）、区域计数定稿
- [ ] 周边三类清单核实：露营地是否仍在营（收费/野营性质）、亲子强度与适龄、打卡点机位是否可靠
